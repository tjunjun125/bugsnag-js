...
Scenario: it works with regexes
  ...
  Then I wait to receive a request
  And the request is a valid browser payload for the error reporting API
  And the event "metaData.ok.details0" equals "[Filtered]"
  And the event "metaData.ok.details1" equals "[Filtered]"
  And the event "metaData.ok.details2" equals "[Filtered]"
